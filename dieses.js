...
single_symptom.addEventListener("submit",(e)=>{
  ...
  for(let i=0;i<1;i++){
    result.childNodes[1].innerHTML=` you can take ${dispaly_single_dieses[i]} <br/> and  ${dispaly_single_dieses[i+1]}`
  }
  for(let i=0;i<1;i++){
    result.childNodes[3].innerHTML=` discription  <br/>${dispaly_single_dieses[i]} <br/> ${dispaly_single_dieses[i+1]}`
  }
